...
/**
 * Servlet implementation class ModifyYearProCtrl
 */
@WebServlet("/ModifyYearProCtrl")
public class ModifyYearProCtrl extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int cnt = dao.modifyYear(dto);
		
		if(cnt>=1){
			response.sendRedirect("GetYearListCtrl.do");
		} else {
			response.sendRedirect("ModifyYearCtrl?no="+no);
		}
	}
